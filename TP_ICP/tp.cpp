...
void ICP(std::vector<Vec3> const & ps , std::vector<Vec3> const & nps ,
	...
	Vec3 cs = GetCentroid(ps);
	Vec3 ct = GetCentroid(qs);

    Mat3 Covariance = CovarianceMatrix(ps,qs,cs,ct,qsKdTree);

// On passe de M a R avec le set rotation qui fait le svd
    Covariance.setRotation();

    for(int i = 0 ; i < ps.size();i++){
        positions3[i] = ps[i];
        positions3[i] -= cs;
        positions3[i] = Covariance * ps[i];
        positions3[i] += ct;
    }
    translation = ct -cs;

    for(unsigned int it = 0 ; it < nIterations-1 ; ++it) {
